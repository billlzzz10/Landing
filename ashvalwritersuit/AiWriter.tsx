...
const AiWriter: React.FC<AiWriterProps> = ({
  ...
  const optionStyle = isDarkTheme 
    ...
    : { backgroundColor: currentTheme.name.toLowerCase().includes('paper') ? '#F9FAFB' : '#EFF6FF', color: currentTheme.name.toLowerCase().includes('paper') ? '#1F2937' : '#334155' }; 
      ...
      <div className="mb-4">
        ...
        <div className="relative">
          <select
            ...
            style={{ backgroundImage: `url("${selectArrowSVG}")`, backgroundSize: '1.25em', color: isDarkTheme ? currentTheme.inputText : '#1F2937' }}
         ...
         <div className="mb-4">
            ...
            >
                {allProjectLoreEntries.map(lore => (
                    <option key={lore.id} value={lore.id} style={optionStyle}>
