        ...
        {!isEditing && allTemplates.length > 0 && (
          <div className="mb-4">
            ...
            <select
              ...
              defaultValue=""
            >
              <option value="" className={optionBgClass}>-- ไม่ใช้แม่แบบ --</option>
              {allTemplates.some(t => !t.isUserTemplate) && (
                <optgroup label="แม่แบบของระบบ" className={optionBgClass}>
                  {allTemplates.filter(t => !t.isUserTemplate).map(template => (
                    <option key={template.id} value={template.id} className={optionBgClass}>
                      {template.name}
                    </option>
                  ))}
                </optgroup>
              ...
              {allTemplates.some(t => t.isUserTemplate) && (
                <optgroup label="แม่แบบของคุณ" className={optionBgClass}>
                    {allTemplates.filter(t => t.isUserTemplate).map(template => (
                    <option key={template.id} value={template.id} className={optionBgClass}>
                        {template.name}
                    </option>
        ...
        <div className="space-y-4 flex-grow flex flex-col min-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            >
              ...
              <option value="chapter" className={optionBgClass}>บท (Chapter)</option>
