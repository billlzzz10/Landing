...
import express, { Response as ExpressResponse, Request as ExpressRequest, RequestHandler as ExpressRequestHandler, NextFunction as ExpressNextFunction } from "express";
...
// Middleware
app.use(cors({ origin: true }) as ExpressRequestHandler);
app.use(express.json() as ExpressRequestHandler);

const DEFAULT_MODEL_NAME_FUNCTIONS = 'gemini-2.5-flash-preview-04-17';
const STREAM_ERROR_MARKER_FUNCTIONS = "[[STREAM_ERROR]]";

const getAiClientFunctions = () => {
  const apiKey = process.env.GEMINI_APIKEY;
  if (!apiKey) {
    console.error("Gemini API Key (GEMINI_APIKEY) is not configured in Firebase Functions environment variables.");
    return null;
  }
  try {
...
app.post('/ai/generate-stream', async (req: ExpressRequest, res: ExpressResponse) => {
  const { systemInstruction, userPrompt, chatHistory, selectedModel } = req.body;
  
  const aiClient = getAiClientFunctions();
...
app.post('/ai/generate-subtasks', async (req: ExpressRequest, res: ExpressResponse) => {
  const { taskTitle, taskCategory, selectedModel } = req.body;
  const aiClient = getAiClientFunctions();
  const modelToUse = selectedModel || DEFAULT_MODEL_NAME_FUNCTIONS;
...
export const api = onRequest((request: FirebaseRequest, response: ExpressResponse) => {
  // Cast Firebase's Request to Express's Request and ExpressResponse for Express app.
  // This is a common workaround for integrating Express apps with Firebase Functions.
  app(request as any as ExpressRequest, response as any as ExpressResponse);
});
